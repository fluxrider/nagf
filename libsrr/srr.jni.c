...
JNIEXPORT jobject JNICALL Java_srr_init(JNIEnv * env, jclass _c, jstring jstring_name, jint length, jboolean is_server, jboolean use_multi_client_lock, jdouble timeout) {
...
JNIEXPORT jstring JNICALL Java_srr_close(JNIEnv * env, jclass _c, jobject o) {
...
JNIEXPORT jobject JNICALL Java_srr_get_1msg_1ptr(JNIEnv * env, jclass _c, jobject o) {
  struct opaque_data * data = (*env)->GetDirectBufferAddress(env, o);
  return (*env)->NewDirectByteBuffer(env, data->mem->msg, data->srr.length);
}

JNIEXPORT jobject JNICALL Java_srr_get_1length_1ptr(JNIEnv * env, jclass _c, jobject o) {
  struct opaque_data * data = (*env)->GetDirectBufferAddress(env, o);
  return (*env)->NewDirectByteBuffer(env, &data->mem->length, 4);
}

JNIEXPORT jstring JNICALL Java_srr_send(JNIEnv * env, jclass _c, jobject o, jint length) {
  struct opaque_data * data = (*env)->GetDirectBufferAddress(env, o);
  const char * error = srr_send_dx(&data->srr, data->mem->msg, length, data->mem->msg, &data->mem->length);
  return error? (*env)->NewStringUTF(env, error) : NULL;
}

JNIEXPORT jstring JNICALL Java_srr_receive(JNIEnv * env, jclass _c, jobject o) {
  struct opaque_data * data = (*env)->GetDirectBufferAddress(env, o);
  const char * error = srr_receive(&data->srr);
  return error? (*env)->NewStringUTF(env, error) : NULL;
}

JNIEXPORT jstring JNICALL Java_srr_reply(JNIEnv * env, jclass _c, jobject o, jint length) {
  struct opaque_data * data = (*env)->GetDirectBufferAddress(env, o);
