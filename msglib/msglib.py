...
class MsgMgr:
  def __init__(self, name, length=8192, is_server=False, use_multi_client_lock=False, timeout=3):
    ...
    self.msg = msglib.msglib_get_mem(msgmgr)
  ...
  def client_send(self, data):
    ...
    # copy data (because client must not retain pointer to shm) TODO unless they are alone
    data = [self.msg[i] for i in range(4, length + 4)] 
    # unlock
    if self.use_multi_client_lock:
  ...
  def server_receive(self):
    ...
    # read header
    length = int.from_bytes(self.msg[:4], byteorder=sys.byteorder, signed=False)
    # return data (servers own the shm until they reply so it's ok not to copy it into a python array)
